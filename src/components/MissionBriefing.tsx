...
export function MissionBriefing({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const awsServices = [
    { name: "Amazon Bedrock", role: "Foundation Models & Gen AI", icon: "🧠" },
    { name: "AWS Lambda", role: "Serverless Compute", icon: "⚡" },
    { name: "Amazon DynamoDB", role: "Database & Analytics", icon: "🗄️" },
    { name: "Amazon S3", role: "Media & Backup Storage", icon: "📦" },
    { name: "AWS API Gateway", role: "API Management", icon: "🔌" },
  ];
        ...
        <div className="flex h-full">
          ...
          <div className="w-[280px] border-r border-white/5 bg-black/40 p-8 hidden lg:flex flex-col gap-8">
            <div className="space-y-2">
              ...
              <h2 className="text-xl font-black tracking-tighter text-white">NextUS</h2>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#FF9900]">AWS-Powered Intelligence</p>
            </div>

            <div className="space-y-1 border-t border-white/5 pt-6">
              <p className="text-[8px] font-black text-white/40 uppercase tracking-widest mb-3">Powered By</p>
              {awsServices.map((srv) => (
                <div key={srv.name} className="text-[9px] text-white/60 hover:text-[#FF9900] transition-colors">
                  <span className="mr-2">{srv.icon}</span>
                  <span className="font-bold">{srv.name.split(' ')[1]}</span>
                </div>
              ))}
            </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <ScrollArea className="flex-1 p-8">
              <div className="space-y-12 pb-12">
                ...
                <section className="space-y-6">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#FF9900] flex items-center gap-3">
                    ☁️ AWS Powered Technology
                  </h3>
                  <div className="grid grid-cols-2 gap-4">
                    {awsServices.map((srv, i) => (
                      <div key={i} className="p-5 rounded-xl bg-[#232F3E]/20 border border-[#FF9900]/20 space-y-3 hover:border-[#FF9900]/40 transition-all">
                        <h4 className="text-[10px] font-black text-[#FF9900] uppercase tracking-widest">{srv.name}</h4>
                        <p className="text-[10px] text-white/60 font-medium">{srv.role}</p>
                      </div>
                    ))}
                  </div>
                </section>
                ...
                <section className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-3">
                      <h4 className="text-[10px] font-black text-[#FF9900] uppercase tracking-widest">AI/ML Layer</h4>
                      <p className="text-[11px] text-white/60 font-medium">Amazon Bedrock for Foundation Models, Gemini Vision AI for detection, and Generative Text Analysis.</p>
                    </div>
                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-3">
                      <h4 className="text-[10px] font-black text-[#FF9900] uppercase tracking-widest">Storage & Data</h4>
                      <p className="text-[11px] text-white/60 font-medium">Amazon S3 for media, DynamoDB for real-time analytics, and scalable backup solutions.</p>
                    </div>
                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-3">
                      <h4 className="text-[10px] font-black text-[#FF9900] uppercase tracking-widest">Compute & APIs</h4>
                      <p className="text-[11px] text-white/60 font-medium">AWS Lambda for serverless workflows, API Gateway for scalable integrations.</p>
                    </div>
                    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-3">
                      <h4 className="text-[10px] font-black text-[#FF9900] uppercase tracking-widest">Frontend Framework</h4>
                      <p className="text-[11px] text-white/60 font-medium">Next.js 15 with AWS Amplify, React Components, and AWS-native deployment patterns.</p>
                    </div>
                  </div>
                </section>
                ...
                {/* Strategy Note */}
                <section className="p-8 rounded-2xl bg-[#FF9900]/5 border border-[#FF9900]/20 relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-[#FF9900]/10 to-transparent opacity-50" />
                  <div className="relative z-10 flex flex-col items-center text-center gap-4">
                    <span className="text-2xl">☁️</span>
                    <div className="space-y-2">
                      <h4 className="text-[14px] font-black text-white uppercase tracking-tighter">AWS-Powered Intelligence Platform</h4>
                      <p className="text-[11px] text-white/40 max-w-xl">NextUS leverages Amazon Bedrock for foundation models, AWS Lambda for serverless workflows, and managed services to empower cities with real-time intelligence that converts raw media into life-saving response protocols.</p>
                    </div>
                  </div>
                </section>
              </div>
